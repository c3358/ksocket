...
ks_rpc_client_proc::ks_rpc_client_proc(uint32_t id)
...
uint32_t ks_rpc_client_proc::id()
{
    return _id;
}
...
void ks_rpc_client::context_connected(struct ks_socket_container* container, struct ks_socket_context* context)
{
    ...
    rpc_client->connection_state(true);
...
void ks_rpc_client::context_disconnected(struct ks_socket_container* container, struct ks_socket_context* context)
{
    ...
    rpc_client->_context = NULL;
    rpc_client->failed_all();
    rpc_client->connection_state(false);
...
void ks_rpc_client::connection_state(bool is_connected)
{
...
bool ks_rpc_client::call(ks_rpc_client_proc* proc)
{
    if (!_connected) {
        failed(proc);
        delete proc;
